...
class CheckoutController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($newAddress['address1'] != null) {
            $address = UseAddress::where('ismain', 1)->count();
            if ($address > 0) {
                $address = UseAddress::where('ismain', 1)->update(['isMain' => 0]);
            }
            $address = new UseAddress();
        ...
        if ($mainAddress) {
            ...
            $order->use_address_id = $mainAddress->id;
            $order->save();

            /**Fetch CartItems as array **/
        ...
        //return  redirect($checkout_session->ur); //throw an error
